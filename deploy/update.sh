#!/bin/bash

# Скрипт для обновления приложения на сервере через git pull
# Использование: ./deploy/update.sh [branch]
# По умолчанию использует main branch

set -e

APP_DIR="/var/www/santa-app"
BRANCH=${1:-main}

echo "🔄 Начало обновления приложения..."
echo "📂 Директория: $APP_DIR"
echo "🌿 Ветка: $BRANCH"

cd $APP_DIR

# Проверка, что мы в git репозитории
if [ ! -d ".git" ]; then
    echo "❌ Ошибка: $APP_DIR не является git репозиторием"
    exit 1
fi

# Сохранение изменений (если есть незакоммиченные, они будут потеряны)
echo "📥 Получение изменений из git..."
git fetch origin
git reset --hard origin/$BRANCH

# Обновление зависимостей backend
echo "📦 Обновление зависимостей backend..."
cd backend
npm install --production

# Обновление зависимостей frontend (нужны все зависимости для сборки)
echo "📦 Обновление зависимостей frontend..."
cd ../frontend
npm install

# Пересборка frontend
echo "🏗️  Пересборка frontend..."
# Очищаем кэш Next.js перед сборкой
if [ -d ".next" ]; then
    echo "🧹 Очистка кэша Next.js..."
    rm -rf .next
fi
npm run build

# Проверка, что сборка прошла успешно
if [ ! -f ".next/prerender-manifest.json" ]; then
    echo "❌ Ошибка: сборка frontend не завершилась успешно"
    exit 1
fi
echo "✅ Frontend успешно собран"

# Перезапуск приложения через PM2
echo "🔄 Перезапуск приложения..."
pm2 restart santa-backend
pm2 restart santa-frontend

# Проверка статуса
echo "📊 Статус приложения:"
pm2 status

echo ""
echo "✅ Обновление завершено!"
echo "🌐 Проверьте работу сайта: https://santa.richislav.com"
